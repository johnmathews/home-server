...
- `zfs list -o name,type,used,refer,volsize,refreservation` - shows all the ZFS
  datasets (file systems, snapshots, volumes).
...
Each VM should have a quota and reservation equal to the OS disk space.

- **zpool quota** = maximum space the VM can use
- **zpool reservation** = minimum space the VM will ever have access to, even
...
#### For Volumes
- `zfs set volsize=32G rpool/data/vm-XXX-disk-1` - maximum size (equivalent to quota).
- `zfs set refreservation=10G rpool/data/vm-XXX-disk-1` - minimum guaranteed space.
