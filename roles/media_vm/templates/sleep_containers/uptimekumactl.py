...
def _should(level:str)->bool: return LEVELS.get(level,20) >= LOG_THRESH
def _clean(v: Any) -> str: return str(v).replace(" ", "_")
def logfmt(level="info", **kvs):
    if not _should(level): return
...
def with_retries(desc: str, func, *args, retries: int, delay_s: int):
    ...
    while True:
        ...
        try:
            out = func(*args)
            logfmt("debug", system="kumactl", event=f"{desc}_ok", attempt=attempt, duration_s=int(time.time()-t0))
            return out
        except Exception as e:
            logfmt("warn", system="kumactl", event=f"{desc}_failed", attempt=attempt, duration_s=int(time.time()-t0), err=e)
            if attempt >= retries:
                logfmt("error", system="kumactl", event=f"{desc}_giving_up", attempts=attempt, total_duration_s=int(time.time()-start_all))
                raise
            attempt += 1
            time.sleep(delay_s)
...
def main():
    ...
    if not url or not user or not pwd:
        logfmt("error", system="kumactl", event="missing_env", url=bool(url), username=bool(user), password=bool(pwd))
        sys.exit(2)

    mapping = load_map(map_path) if os.path.exists(map_path) else {}
    try:
        from uptime_kuma_api import UptimeKumaApi
        api = with_retries("login", UptimeKumaApi, url, retries=retries, delay_s=delay_s)
        with_retries("authenticate", api.login, user, pwd, retries=retries, delay_s=delay_s)

        mid = find_monitor_id(api, args.container, mapping)
        if mid is None:
            logfmt("warn", system="kumactl", container=args.container, event="monitor_not_found")
            try: api.disconnect()
        ...
        if args.action == "pause":
            res = with_retries("pause_monitor", api.pause_monitor, mid, retries=retries, delay_s=delay_s)
            logfmt("info", system="kumactl", container=args.container, event="paused", monitor_id=mid, msg=res.get("msg","ok"), duration_s=int(time.time()-t0))
        else:
            res = with_retries("resume_monitor", api.resume_monitor, mid, retries=retries, delay_s=delay_s)
            logfmt("info", system="kumactl", container=args.container, event="resumed", monitor_id=mid, msg=res.get("msg","ok"), duration_s=int(time.time()-t0))
        ...
        logfmt("error", system="kumactl", container=args.container, event="exception", err=e)
        sys.exit(1)
