...
#   - During PAUSE/STOP: Shares are disabled AFTER containers are paused/stopped
#   - During UNPAUSE/START: Shares are enabled BEFORE containers are unpaused/started
#
...
manage_nfs_smb_shares() {
  ...
  case "$action" in
  disable)
    # Safe to disable shares after stopping containers
...
# For UNPAUSE/START: enable shares before touching containers
if [[ "$ACTION" == "unpause" || "$ACTION" == "start" ]]; then
  msg ""
  msg "========================================"
  msg "PHASE 0: ENABLE NFS/SMB SHARES"
  msg "========================================"
  msg ""
  # No container list here – truenas-shares.sh will only enable shares,
  # and skip container health checks.
  manage_nfs_smb_shares enable
  msg ""
fi
