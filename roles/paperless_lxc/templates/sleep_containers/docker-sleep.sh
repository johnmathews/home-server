...
should_log() { local want; want=$(level_num "$1"); [[ $want -ge $LOG_THRESH ]]; }
_log() {
...
is_within_quiet_window() {
  # If not configured, always "true"
  [[ -z "${QUIET_START}" || -z "${QUIET_END}" ]] && return 0
  ...
  if (( end_s <= start_s )); then
    ...
    local tomorrow; tomorrow=$(date -d "${today} +1 day" +%F)
...
# Skip work if outside window for pause
if [[ "$ACTION" == "pause" ]] && ! is_within_quiet_window; then
...
is_busy() {
  local name="$1"
  local plug; plug="$(plugin_for "$name")"
  load_common
